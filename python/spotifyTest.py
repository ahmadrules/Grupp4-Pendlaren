...
async def fillPlaylist(genre, length, accessToken, fromStop, toStop):
    global access_token
    access_token = accessToken

    await getUserID()

    playlistID = await createPlaylist(fromStop, toStop, genre)

    uris = []
    totalPlaylistLength = 0

    while totalPlaylistLength < length:
        tracks = await searchForTracks(genre)
        for track in tracks:
            data = track['uri']
            totalPlaylistLength = totalPlaylistLength + int(track['track_duration'])
            uris.append(data)
            if totalPlaylistLength >= length:
...
async def createPlaylist(fromStop, toStop, genre):
    url = "https://api.spotify.com/v1/users/" + userID + "/playlists"
    headers = {'Authorization': 'Bearer ' + access_token, 'Content-Type': 'application/json'}

    date = datetime.datetime.now()
    dateFormatted = date.strftime("%d/%m/%Y")
    data = {"name": dateFormatted + " " + genre,
            "description": "Spellista för resa från " + fromStop + " till " + toStop + "// Pendlaren"}
